...
FROM task
INNER JOIN user WHERE user.email LIKE "%@spotify.com";

-- Get all the tasks for Donald Duck with status "Not Started"
SELECT
task.title, status.name, user.name
FROM task
INNER JOIN user INNER JOIN status WHERE user.name = "Donald Duck" AND status.name = "Not started";

-- Get all the tasks for Maryrose Meadows that were created in September
SELECT
task.title, user.name, created
FROM task
INNER JOIN user WHERE user.name = "Maryrose Meadows" AND month(created)=9; 
